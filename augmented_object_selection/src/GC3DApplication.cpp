...
const Rect GC3DApplication::DEFAULT_RECT(0,0,0,0);
...
{
  static_cast<GC3DApplication*>(obj)->mouseClick(event, x, y, flags);
}
...
GC3DApplication::GC3DApplication(const string& _name, const Mat& _img, const Mat& _depth_image)
  : win_name_(_name)
{
  ...
  ROS_DEBUG("GC3DApplication: creating window");

  // Initialize window
  imageIs(_img,_depth_image);
  winImageStateIs(GC3DApplication::UPDATED);
...
void 
GC3DApplication::initializedIs(bool _init)
{
  if (_init)
    initialized_=_init;
  else
...
void GC3DApplication::imageIs(const Mat& _image, const Mat& _depth_image)
{
    if( _image.empty() ) {
 ...
 * Grabcut mask. */
void GC3DApplication::setRectInMask()
{
    assert( !mask_.empty() );
    mask_.setTo( GC_BGD );
...
void GC3DApplication::setLblsInMask( int flags, Point p, bool isPr )
{
...
void GC3DApplication::mouseClick( int event, int x, int y, int flags)
{
